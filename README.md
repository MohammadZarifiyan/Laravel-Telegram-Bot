...
```shell
composer require mohammad-zarifiyan/laravel-telegram-bot:^12.1
```
...
Every Command Handler should return `MohammadZarifiyan\Telegram\Enums\Signal`. If it returns `Signal::Exit`, update processing stops; If the Command Handler returns `Signal::Continue`, the next Command Handlers will be executed.
...
Every Breaker should return `MohammadZarifiyan\Telegram\Enums\Signal`. If it returns `Signal::Exit`, update processing stops; If the breaker returns `Signal::Continue`, the next breakers will be executed.
